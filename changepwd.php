...
$query=("select password from user_signup where login_id='$login_id'");
...
if($n==$oldpwd)
{
	...
	{
		$query1=("update user_signup set password='$newpwd' where login_id='$login_id';") or die(mysql_error());
		$result1=mysql_query($query1);
		if($result)
		echo "<p id=ch>Password successfully changed</p><p><img src=http://localhost/voters/photos/loginicon.gif></p>";
		header("refresh:2; url=http://localhost/voters/changepwd.html");
	}
}
else{
echo "<p id=nch><img src=http://localhost/voters/photos/wrong.GIF>&nbsp&nbsp;your old password is incorrect</p>";
header("refresh:3; url=http://localhost/voters/changepwd.html");
}
}
else
{
